...
class KGNNLS(KnowledgeRecommender):
    ...
    def get_neighbors(self, items):
        ...
        index = torch.flatten(entities[0])
    ...
    def aggregate(self, user_embeddings, entities, relations):
        ...
        neighbor_vectors = entity_vectors[1].reshape(shape)
        neighbor_relations = relation_vectors[0].reshape(shape)
        ...
        output = self.linear_layers[0](output)
        # [batch_size, -1, dim]
    ...
    def label_smoothness_predict(self, user_embeddings, user, entities, relations):
        ...
        masks = reset_masks[0]
        self_labels = entity_labels[0]
        neighbor_labels = entity_labels[1].reshape(self.batch_size, -1, self.neighbor_sample_size)
        neighbor_relations = relation_vectors[0].reshape(
