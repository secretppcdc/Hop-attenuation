# -*- coding: utf-8 -*-
# @Time   : 2020/10/3
# @Author : Sun Guangbiao
# @Email  : sung0012@e.ntu.edu.sg
...
class KGNNHALS(KnowledgeRecommender):
    ...
    def __init__(self, config, dataset):
        super(KGNNHALS, self).__init__(config, dataset)
        ...
        # number of allowed distance when computing entity representation
        self.distance = config['distance']
        self.hop_decay = config['hop_decay']
    ...
    def aggregate(self, user_embeddings, entities, relations):
        ...
        for i in range(self.distance):
            ...
            for hop in range(self.distance - i):
                ...
                )*(self.hop_decay**(hop+1))  # [batch_size, -1, dim]
    ...
    def forward(self, user, item):
        ...
        # {[batch_size, 1], [batch_size, n_neighbor], [batch_size, n_neighbor^2], ..., [batch_size, n_neighbor^distance]}
    ...
    def calculate_loss(self, interaction):
        ...
        #user_e = self.user_embedding(user)
        entities, relations = self.get_neighbors(items)

        #predicted_labels = self.label_smoothness_predict(user_e, users, entities, relations)
